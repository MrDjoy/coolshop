...
$keywords=$_REQUEST['keywords']?$_REQUEST['keywords']:null;//本页面GET过来的keywords
$where=$keywords?"where p.pName like '%{$keywords}%'":null;//运用like模式匹配进行搜索
//得到数据库中所有商品
$sql="select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName from coolshop_pro as p join coolshop_cate c on p.cId=c.id {$where}  ";
...
$sql="select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName from coolshop_pro as p join coolshop_cate c on p.cId=c.id {$where} {$orderBy} limit {$offset},{$pageSize}";
$rows=fetchAll($sql);
?>
<!doctype html>
<html>
                            ...
                            <tr>
                                ...
                                <td align="center">
                                                ...
                                                <table class="table" cellspacing="0" cellpadding="0">
                                                    ...
                                                    <tr>
                                                        <td  width="20%"  align="right">官网价格</td>
